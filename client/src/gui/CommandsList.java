    ...
    {      
        ...
        if (com == List.SELECT_COMMAND)
        {
            try 
            {
                ...
                /* append client mode to be able to distinguish between application modes 
                 * "Ctrl + remote mode" denotes the mode the application is running
                 * [07.01.10, SBe]
                 */
                if(remote.mode == remoteclient.Remote.APP_MODE)
                {
                    remote.bluetooth.SendData("K17");
                    remote.bluetooth.SendData("K32");
                }
                
                for (int x = 0; x < keys.length; x++) 
                {
                    String toSendPressed = new String("K");
                    toSendPressed += keys[x];
                    remote.bluetooth.SendData(toSendPressed);                    //remote.bluetooth.SendData("K" + keys[x]);
                }
                
                /* for symmetry purposes [07.01.10, SBe]*/
                if(remote.mode == remoteclient.Remote.APP_MODE)
                {
                    remote.bluetooth.SendData("SK17");
                    remote.bluetooth.SendData("SK32");
                }
                
                for (int x = 0; x < keys.length; x++) 
                {
                    String toSendReleased = new String("SK");
                    toSendReleased += keys[x];
                    remote.bluetooth.SendData(toSendReleased);
                }
            } 
